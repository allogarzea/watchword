...
const checkForErrorsInParameters = (
	...
	// NUMBER WARNING
	if (
		compareValueWithMinimumValue(minNumbers, recommendedValues.numbers.minimum)
	) {
		log(chalk.yellow(PARAMS_WARNING.NUMBER_IS_LESS_THAN_RECOMMENDED));
		return 'warn';
	}
	if (
		compareValueWithMaximumValue(minNumbers, recommendedValues.numbers.maximum)
	) {
	...
	if (
		compareValueWithMinimumValue(
			...
			recommendedValues.specialCharacters.minimum
		)
	) {
		log(
			chalk.yellow(PARAMS_WARNING.SPECIAL_CHARACTERS_IS_LESS_THAN_RECOMMENDED)
		);
		return 'warn';
	}
	if (
		...
		log(
			chalk.yellow(
				PARAMS_WARNING.SPECIAL_CHARACTERS_IS_GREATER_THAN_RECOMMENDED
			)
		);
		return 'warn';
	} else {
...
const validateParams = (passwordParams: PasswordParameters): any => {
	...
	switch (response) {
		case 'stop':
			return false;
		case 'warn': {
			confirmationQuestion(passwordParams);
			return;
		}
		case 'keep':
		default:
...
export { validateParams, checkForErrorsInParameters };
